						...
						<div class="main-20">
							...
							<div style="text-align:center;margin-bottom:15px;">
								...
								<select id="type_id" class="no-shadow">
									...
									<c:forEach items="${typeList}" var="typeInfo" varStatus="status" >
										<option value="${typeInfo.labType_Id}" <c:if test="${typeInfo.labType_Id==labType_Id}">selected</c:if>>${typeInfo.typeName}</option>
									</c:forEach>
								</select>
								<!-- 范围日期检索 -->
								<input type="text" id="date2" class="date" style="width: 300px;" value="" readonly/>
								<!-- 标题检索 -->
								<input type="text" class="text" id="title" value="${keyWord}" placeholder="检索实验室名称" />
								<!-- 点击查询按钮 -->
							...
							<div class="operation-wrap">
								<div class="buttons-wrap">
									<a href="edit.action">
										<button id="add" class="button blue"><span class="icon-plus"></span>  新增实验室 </button>
									</a>
								</div>
							</div>
									...
									<c:choose>
										...
										<c:otherwise>
											<c:forEach items="${pageInfo.list}" var="entity" varStatus="status" >
												<tr>
													<td><input type="checkbox" class="fill listen-1-2" name="lab_Id" value="${entity.lab_Id}"/> </td>
													<td>${status.index+1}</td>
													<td>${entity.labName}</td>
													<td>${entity.typeName}</td>
													<td>${entity.doorName}</td>
													<td>${entity.labna}</td>
													<td>${entity.updateTime}</td>
													<td>
														<a href="edit.action?lab_Id=${entity.lab_Id}">
									...
									<div class="block no-shadow">
										...
										<div class="main" style="margin-bottom:100px;">
											...
											<select id="type_Id2" class="no-shadow">
												<c:forEach items="${typeList}" var="typeInfo" varStatus="status" >
													<option value="${typeInfo.labType_Id}">${typeInfo.typeName}</option>
												</c:forEach>
											</select>
											<br/>
...
<script>
	...
	function search(pageNum) {
		...
		//实验室类型
		var labType_Id = $("#type_id").val();
		
		//实验室名称
		var keyWord = $("#title").val();
		
		//分页
		window.location.href = "list_Lab_normal.action"
		+ "?pageNum="+pageNum
	...
	$("#submit").click(function() {
		...
		//id：把选中的id保存到数组中
		$(':checkbox[name="lab_Id"]:checked').each(function() {
			idArr.push($(this).val());
		});
